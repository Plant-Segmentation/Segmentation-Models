...
flags.DEFINE_list('eval_crop_size', '901,901',
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    iou_v, update_op1 = my_metrics.iou(
        predictions, labels, dataset.num_of_classes, weights=weights)
    for index in range(0, dataset.num_of_classes):
        tf.summary.scalar(predictions_tag+'_class'+str(index), iou_v[index])
        
    # iu, mean_iu = evaluate(predictions, labels, dataset.num_of_classes)

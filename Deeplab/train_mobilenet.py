...
flags.DEFINE_string('train_logdir', "checkpoints_mobilenet/",
...
flags.DEFINE_float('base_learning_rate', 1e-3, #6e-3,
...
flags.DEFINE_integer('training_number_of_steps', int(60000),
...
flags.DEFINE_float('weight_decay', 1e-4, #4e-5
...
flags.DEFINE_string('tf_initial_checkpoint', 'deeplabv3_mnv2_pascal_trainval/model.ckpt-30000', #'ckpt/model.ckpt-58337', #
                    'The initial checkpoint in tensorflow format.')

# Set to False if one does not want to re-use the trained classifier weights.
flags.DEFINE_boolean('initialize_last_layer', False,#True,
                     'Initialize the last layer.')

flags.DEFINE_boolean('last_layers_contain_logits_only', False,
                     'Only consider logits as last layers or not.')

flags.DEFINE_integer('slow_start_step', 37000,
...
flags.DEFINE_integer(
    'hard_example_mining_step', 37000,
...
def main(unused_argv):
  ...
  print("min resize val: ", FLAGS.min_resize_value)
  print("max resize val: ", FLAGS.max_resize_value)
  print("resize factor: ", FLAGS.resize_factor)        
      ...
      with tf.contrib.tfprof.ProfileContext(
        ...
        with tf.train.MonitoredTrainingSession(
          ...
          while not sess.should_stop():
            ...
            # if training_loss[0] < min_loss:
            #   saver = tf.train.Saver()
            #   saver.save(sess, 'best_chkpt/')

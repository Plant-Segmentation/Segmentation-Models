...
class BaseDataSet(Dataset):
    ...
    def _augmentation(self, image, label):
        ...
        h, w, _ = image.shape
        # Rotate the image with an angle between -15 and 15
        if self.rotate:
        ...
        if self.crop_size:
            if h < w:
                self.crop_h, self.crop_w = (self.crop_size, int(self.crop_size * w / h))
            else:
                self.crop_h, self.crop_w = (int(self.crop_size * h / w), self.crop_size)

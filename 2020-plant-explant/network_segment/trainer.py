...
class Trainer(BaseTrainer):
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, train_logger=None, prefetch=True):
        ...
        self.print_step = config['trainer'].get('print_per_iter', int(np.sqrt(self.train_loader.batch_size)))
        if config['trainer']['print_per_iter']:
            self.print_step = int(self.print_step / self.train_loader.batch_size) + 1
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            if batch_idx % self.print_step == 0:
                prt_format_str = 'TRAIN ({}) | Loss: {:.3f} | Acc {:.2f} mIoU {:.2f} | B {:.2f} D {:.2f} |'
                tbar.set_description(prt_format_str.format(
